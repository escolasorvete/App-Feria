...
export class App extends Component {
  ...
  getSegmentsData() {
    fetch('http://localhost:8000/api/json')
      ...
      .then(data => {
        this.setState({ segmentsData: data });
      })
      .catch(error => {
        console.error('Error al obtener los datos de los segmentos:', error);
      });
  }
  ...
  render() {
    const { segmentsData, show } = this.state;

    if (!segmentsData) {
      return <div>Cargando datos de los segmentos...</div>;
    }
    ...
    return (
        ...
        >
          ...
          {show && <Confetti width={1080} height={1920} />}
